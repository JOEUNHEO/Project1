 ...
 */
@WebFilter({"/users1/private/*", "/board/*", "/file/*"})// private 폴더내에 있는 jsp로 요청하기 전에 Filter 실행!
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		if(session.getAttribute("id") == null) {
			//로그인 페이지의 경로 구성
			String path = cPath+"/users1/login_form.jsp?url="+url;//원래 요청 경로를 달고 가는 것임!(로그인 하고 원래 경로로 이동하기 위해)
			//로그인 페이지로 이동 시킨다.
